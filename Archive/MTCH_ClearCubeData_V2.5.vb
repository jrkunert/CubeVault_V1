...
Namespace OneStream.BusinessRule.Finance.MTCH_ClearCubeData
	...
	Public Class MainClass
		...
		Private Const BR_VERSION As String = "2.5.1"
...
#Region "Restore"
		...
		Private Shared Sub ExecuteRestore( _
			...
			Try
				...
				Using sr As New StreamReader(ctx.CsvFile)
					...
					Dim rec As RestoreRecord =
						ReadRestoreRecord(line)

					Log(si, "")
					Log(si, "First Record")
					Log(si, LOG_SEPARATOR)

					Log(si, "Line      : " & rec.LineNo)

					Log(si, "Scenario  : " & rec.Scenario)

					Log(si, "Time      : " & rec.Time)

					Log(si, "Entity    : " & rec.Entity)

					Log(si, "Account   : " & rec.Account)

					Log(si, "Flow      : " & rec.Flow)

					Log(si, "Origin    : " & rec.Origin)
		...
		Private Shared Function ReadRestoreRecord( _
				ByVal line As String) As RestoreRecord
			...
			rec.LineNo = Integer.Parse(fields(0))

			rec.ExportDate = DateTime.Parse(fields(1))

			rec.BRVersion = fields(2)

			rec.Cube = fields(3)

			rec.UserName = fields(4)

			rec.Scenario = fields(5)
			...
			rec.Currency = fields(10)

			rec.Account = fields(11)

			rec.Flow = fields(12)

			rec.Origin = fields(13)
			...
			rec.Amount = Decimal.Parse(fields(24))
