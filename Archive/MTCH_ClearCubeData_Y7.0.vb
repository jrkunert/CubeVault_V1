...
Namespace OneStream.BusinessRule.Finance.MTCH_ClearCubeData
	...
	Public Class MainClass
		...
		Private Const BR_VERSION As String = "2.6.1"
...
Log(si, "")
Log(si, "ExecuteRestore = " & ctx.ExecuteRestore.ToString())
Log(si, "Preview = " & ctx.Preview.ToString())
		...
		Private Shared Sub ReadCsv( _
			...
			Using parser As New Microsoft.VisualBasic.FileIO.TextFieldParser(ctx.CsvFile)
				...
				If ctx.ExecuteRestore AndAlso _
				   ctx.MemberScriptAndValues.Count > 0 Then

					Log(si, "")
					Log(si, "Submitting " &
						ctx.MemberScriptAndValues.Count.ToString("#,##0") &
						" record(s) to OneStream...")

					Log(si, "")
					Log(si, "First Script:")
					Log(si, ctx.MemberScriptAndValues(0).Script)

					Log(si, "Amount = " &
						ctx.MemberScriptAndValues(0).Amount.ToString())
					...
					Log(si, "")
					Log(si, "XFResult Type = " &
						objXFResult.GetType().FullName)
