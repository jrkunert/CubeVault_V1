...
'=========================================================
' Build Filter Script (Version 6)
'=========================================================


		Private Shared Function BuildFilterScriptV6( _
				ByVal ctx As ExportContext, _
				ByVal si As SessionInfo, _
				ByVal api As FinanceRulesApi, _
				ByVal originName As String) As String
...
#Region "Helper Methods"
		...
		Private Shared Sub AddExpandedIfPresent( _
				...
				Log(si, "")
				Log(si, "*** AddExpandedIfPresent CALLED ***")

			If String.IsNullOrWhiteSpace(memberName) Then Exit Sub

			Log(si, "")
			Log(si, "===== AddExpandedIfPresent =====")
			Log(si, "Dimension  : " & dimPrefix)
			Log(si, "Expression : " & memberName)
...
#Region "Helper Classes"
		...
		Private Class ExportContext
			...
			''''''Public MemberFilters As New Dictionary(Of String, DimensionFilter)( _
			''''''	StringComparer.OrdinalIgnoreCase)
			
			'=========================================================
			' Export Member Filters
			'=========================================================
			Public FilterParameters As New Dictionary(Of String, String)( _
				StringComparer.OrdinalIgnoreCase)
...
'''''''''''''''''''''''''''''''''''''
If ctx.FilterParameters.Count > 0 Then

    Log(si, "")
    Log(si, "Export Member Filters")
    Log(si, LOG_SEPARATOR)

    For Each kvp As KeyValuePair(Of String, String) _
        In ctx.FilterParameters
