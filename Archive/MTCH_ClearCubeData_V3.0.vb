...
Namespace OneStream.BusinessRule.Finance.MTCH_ClearCubeData
	...
	Public Class MainClass
		...
		Private Const BR_VERSION As String = "2.7.1"
...
#Region "Restore"
		...
		Private Shared Sub ExecuteRestore( _
			...
			Try
				...
				ctx.Preview = Not ctx.ExecuteRestore
...
'====================================================================
		...
		Private Shared Sub ReadCsv( _
			...
			Using parser As New Microsoft.VisualBasic.FileIO.TextFieldParser(ctx.CsvFile)
				...
				Log(si, "CSV File        : " & ctx.Summary.CsvFile)
				Log(si, "Rows Read       : " & ctx.Summary.TotalRows.ToString("#,##0"))
				Log(si, "Rows Restored   : " & ctx.Summary.RowsRestored.ToString("#,##0"))
				Log(si, "Rows Skipped    : " & ctx.Summary.RowsSkipped.ToString("#,##0"))
				Log(si, "Batches         : " & ctx.Summary.BatchCount.ToString("#,##0"))
				Log(si, "Errors          : " & ctx.Summary.ErrorCount.ToString("#,##0"))

				Dim elapsed As TimeSpan =
					ctx.Summary.EndTime - ctx.Summary.StartTime

				Log(si, "Elapsed Time    : " & elapsed.ToString())
